...
// Add this right after the map initialization at the top of script.js
// Add this code to the top of script.js to run when the page loads. 
// It will check the role and show the admin controls if necessary.
document.addEventListener('DOMContentLoaded', () => {
    ...
    // The table will be handled by populateTable
});
...
// The updated populateTable function in script.js

function populateTable(features) {
  const tbody = document.querySelector("#parcelTable tbody");
  const userRole = getRole(); // Get the current user's role
  const isEditable = userRole === 'admin'; // Check if the user is an admin

  tbody.innerHTML = "";
  features.forEach((f) => {
    const tr = document.createElement("tr");

    // Conditionally add the 'contenteditable' attribute
    tr.innerHTML = `
      <td>${f.properties.ParcelID}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'OccupierID', this.innerText)">${f.properties.OccupierID}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'Area', this.innerText)">${f.properties.Area}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'Land_Use', this.innerText)">${f.properties.Land_Use}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'TaxID', this.innerText)">${f.properties.TaxID}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'SurveyPlan', this.innerText)">${f.properties.SurveyPlan}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'Surname', this.innerText)">${f.properties.Surname}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'Firstname', this.innerText)">${f.properties.Firstname}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'DOB', this.innerText)">${f.properties.dob}</td>
      <td ${isEditable ? 'contenteditable="true"' : ''} onblur="updateProperty(${f.id}, 'SurveyPlan', this.innerText)">${f.properties.Gender}</td>`;

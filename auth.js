    ...
    try {
        ...
        if (!adminUser) {
            ...
            await db.users.add({
                name: 'Administrator',
                username: 'admin',
                password: 'password', // IMPORTANT: Default password, should be changed.
                role: 'admin'
            });
            console.log('Default admin created with username "admin" and password "password".');
        }
    } catch (error) {
...
async function loginAdmin() {
    ...
    const admin = await db.users.where({ username: username, role: 'admin' }).first();

    if (admin && admin.password === password) {
        alert('Admin login successful!');
        sessionStorage.setItem('loggedInUser', admin.username);
        sessionStorage.setItem('userRole', 'admin');
        window.location.href = 'index.html'; // Redirect admin to the main map
    } else {
...
async function loginUser() {
    ...
    if (user) {
        ...
        if (user.password === password) {
            ...
            sessionStorage.setItem('userRole', 'user'); // <-- ADD THIS LINE
